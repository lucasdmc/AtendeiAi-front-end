// Nó "Fazer uma pergunta" do canvas
...
import { AskQuestionDrawer, AskQuestionConfig } from '@/components/drawers/ask-question/AskQuestionDrawer';
...
export const AskQuestionNode = memo(({ id, data, selected }: NodeProps) => {
  const nodeData = (data || {}) as AskQuestionData;
  const { getNodes, setNodes } = useReactFlow();
  const pushHistory = useEditorStore((state) => state.pushHistory);

  const [value, setValue] = useState<AskQuestionConfig>(
    nodeData.value || {
      headerRichText: '',
  ...
  // Atualizar valor
  const handleValueChange = (newValue: AskQuestionConfig) => {
    setValue(newValue);
  ...
  const nodeInfo = {
    ...
    description: `Este bloco permite capturar uma resposta de texto livre do contato.
