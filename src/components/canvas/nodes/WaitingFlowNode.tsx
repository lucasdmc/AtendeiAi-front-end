...
import { WaitingFlowConfig } from '@/types/utilityNodes';

export interface WaitingFlowData {
  value?: WaitingFlowConfig;
  onChange?: (value: WaitingFlowConfig) => void;
}

export const WaitingFlowNode = memo(({ id, data, selected }: NodeProps) => {
  const nodeData = (data || {}) as WaitingFlowData;
  const { getNodes, setNodes } = useReactFlow();
  const pushHistory = useEditorStore((state) => state.pushHistory);
  
  // Garantir que sempre temos um valor padrão
  const defaultValue: WaitingFlowConfig = { enabled: true };
  const [value, setValue] = useState<WaitingFlowConfig>(
  ...
  const handleDuplicate = useCallback(() => {
    console.log(`[WaitingFlowNode] Duplicate node: ${id}`);
  }, [id]);

  const handleDelete = useCallback(() => {
    console.log(`[WaitingFlowNode] Delete node: ${id}`);
  }, [id]);

  const nodeInfo = {
    title: 'Fluxo de espera',
    description: `Ativa ou desativa o fluxo de espera na conversa.

Caso a organização tenha configurado um fluxo de espera a ser executado. Se Ativo, o fluxo será executado após o atendente não responder a última mensagem do contato no tempo determinado.`,
  };
...
WaitingFlowNode.displayName = 'WaitingFlowNode';
