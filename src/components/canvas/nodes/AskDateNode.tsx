/**
 * Canvas Node: Perguntar por uma data
 */
...
import { AskDateConfig } from '@/types/askDate';
import { AskDateDrawer } from '@/components/drawers/ask-date/AskDateDrawer';
...
export interface AskDateData {
  value?: AskDateConfig;
  onChange?: (value: AskDateConfig) => void;
}

export const AskDateNode = memo(({ id, data, selected }: NodeProps) => {
  const nodeData = (data || {}) as AskDateData;
  const { getNodes, setNodes } = useReactFlow();
  const pushHistory = useEditorStore((state) => state.pushHistory);
  
  const [value, setValue] = useState<AskDateConfig>(
    nodeData.value || {
      headerRichText: '',
  ...
  const handleDuplicate = useCallback(() => {
    console.log(`[AskDateNode] Duplicate node: ${id}`);
  }, [id]);

  const handleDelete = useCallback(() => {
    console.log(`[AskDateNode] Delete node: ${id}`);
  }, [id]);

  const nodeInfo = {
    title: 'Perguntar por uma data',
    description: `Este bloco permite capturar uma data do contato.

Configure uma pergunta, escolha o formato da data (DD/MM/YYYY, MM/DD/YYYY, etc.), defina onde salvar a resposta e configure fluxos alternativos para respostas inválidas ou timeout.

O sistema valida se a entrada é uma data válida no formato especificado e permite até 3 tentativas antes de seguir para o fluxo alternativo.

Nota: O WhatsApp não exibe um seletor de calendário visual. O usuário deve digitar a data no formato correto.`,
  };
    ...
    <>
      ...
      >
        ...
        {value.invalidFlowEnabled && connectorPositions.invalid > 0 && (
          ...
          >
            <NodeConnector
              ...
              label="Data inválida (após 3 tentativas)"
            />
          </div>
        )}
        ...
        <div className="pt-3 pb-4 px-4 space-y-3">
          ...
          {value.dateFormat && (
            <div className="flex items-center gap-2">
              <span className="text-[12px] text-neutral-500">Formato:</span>
...
AskDateNode.displayName = 'AskDateNode';
