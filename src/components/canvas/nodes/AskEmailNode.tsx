// Nó "Perguntar por um e-mail" do canvas
...
import { AskEmailDrawer, AskEmailConfig } from '@/components/drawers/ask-email/AskEmailDrawer';
...
export interface AskEmailData {
  value?: AskEmailConfig;
  onChange?: (value: AskEmailConfig) => void;
}

export const AskEmailNode = memo(({ id, data, selected }: NodeProps) => {
  const nodeData = (data || {}) as AskEmailData;
  const { getNodes, setNodes } = useReactFlow();
  const pushHistory = useEditorStore((state) => state.pushHistory);

  const [value, setValue] = useState<AskEmailConfig>(
    nodeData.value || {
      headerRichText: '',
  ...
  }, [value.invalidFlowEnabled, value.noResponseEnabled, value.headerRichText, value.validationErrorMessage]);
  ...
  // Atualizar valor
  const handleValueChange = (newValue: AskEmailConfig) => {
    setValue(newValue);
  ...
  const hasErrorMessage = value.validationErrorMessage && value.validationErrorMessage.trim().length > 0;
  ...
  const nodeInfo = {
    ...
    description: `Este bloco permite capturar e validar o e-mail do contato.

O sistema valida automaticamente se o texto fornecido é um endereço de e-mail válido. Configure uma mensagem de erro personalizada para quando a validação falhar.

Após 3 tentativas inválidas, você pode seguir para um fluxo alternativo ou o sistema enviará a mensagem de erro configurada.`,
  };
    ...
    <>
      ...
      >
        ...
        {value.invalidFlowEnabled && connectorPositions.invalid > 0 && (
          ...
          >
            <NodeConnector
              ...
              label="E-mail inválido (após 3 tentativas)"
            />
          </div>
        )}
        ...
        <div className="pt-3 pb-4 px-4 space-y-3">
          ...
          {/* Info sobre mensagem de erro */}
          {!value.invalidFlowEnabled && hasErrorMessage && (
            <div className="flex items-center gap-1 text-[12px] text-emerald-600">
              <span>✓ Mensagem de erro configurada</span>
            </div>
          )}

          {!value.invalidFlowEnabled && !hasErrorMessage && (
            <p className="text-[12px] text-orange-600">
              Configure uma mensagem de erro de validação.
            </p>
          )}
